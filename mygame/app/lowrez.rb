...
# Emulation of a 256x144 canvas.

LOWREZ_SIZE_W          = 256
LOWREZ_SIZE_H          = 144
LOWREZ_ZOOM            = 5
LOWREZ_X_OFFSET        = (1280 - LOWREZ_SIZE_W * LOWREZ_ZOOM).half
LOWREZ_Y_OFFSET        = (720 - LOWREZ_SIZE_H * LOWREZ_ZOOM).half
...
LOWREZ_FONT_UNI_BOLD = {
  path: 'fonts/uni05_63.ttf',
  size: {
    small: -2,
    medium: 7,
    large: 16
...
# Class to handle low resolution
class LowrezOutputs
  ...
  def background_color= opts
    ...
    outputs_lowrez.solids << [0, 0, LOWREZ_SIZE_W, LOWREZ_SIZE_H, @background_color]
  end

  def outputs_lowrez
    return @args.outputs if Kernel.tick_count <= 0

    @args.outputs[:lowrez].transient!
  end
  ...
  def mouse_inside_rect?(rect)
    m = mouse
    m.x >= rect.x && m.x <= rect.x + rect.w && m.y >= rect.y && m.y <= rect.y + rect.h
  end
...
module GTK
  ...
  class Runtime
    ...
    def tick_core
      ...
      @args.outputs
           .sprites << { x: LOWREZ_X_OFFSET,
                         y: LOWREZ_Y_OFFSET,
                         w: LOWREZ_SIZE_W * LOWREZ_ZOOM,
                         h: LOWREZ_SIZE_H * LOWREZ_ZOOM,
                         source_x: 0,
                         source_y: 0,
                         source_w: LOWREZ_SIZE_W,
                         source_h: LOWREZ_SIZE_H,
                         path: :lowrez }
    end
  end
end
